...
par(mfrow = c(2,2))
with(plot_data, {
      plot(d_Date_Time, Global_active_power, type = "l", ylab = "Global active power", xlab="")
      plot(d_Date_Time, Voltage, type = "l", ylab = "Voltage", xlab="datetime")
      plot(d_Date_Time, Sub_metering_1, type = "l", ylab = "Energy sub metering", xlab="")
        lines(d_Date_Time, Sub_metering_2, col = "red")
        lines(d_Date_Time, Sub_metering_3, col = "blue")
        legend("topright", legend=c("Sub_metering_1","Sub_metering_2", "Sub_metering_3"), col=c("black", "blue","red"), lwd=c(1, 1, 1))
      plot(d_Date_Time, Global_reactive_power, type = "l", xlab="datetime")        
 })
dev.off()
